...
### Deployed
- [upstream-sync] **Siebter Upstream-Merge LIVE auf DEV (2026-05-04)** — Helm Rev 69, Chart 0.4.48, Image aus Commit `181e4aa24`. Alle 14 Onyx-Pods Running, `/api/health` 200, `/api/ext/health/deep` 200 (postgres+redis+opensearch alle OK), VOeB-Branding aktiv (`/api/enterprise-settings` liefert `application_name="VÖB Service Chatbot Dev"` + `use_custom_logo=true`). **Alembic-Recovery diesmal NICHT noetig:** `alembic_version = d8a1b2c3e4f5` direkt nach dem Container-Boot — die zwei neuen Upstream-Migrationen (`14162713706c`, `31bd8c17325e`) fuegen nur neue Tabellen hinzu (`index_attempt_metric`, `targeted_reindex`-Schema), modifizieren keine bestehenden Spalten die der Boot-Code sofort liest. Ergo: `alembic upgrade head` lief automatisch durch, kein CrashLoopBackOff. **Pattern-Aenderung gegenueber Sync #5/#6:** Recovery ist nur noetig, wenn die Upstream-Migrationen Spalten-Aenderungen auf bestehenden Tabellen machen, die im Boot-Code gelesen werden (Sync #5: `account_type`, `is_listed`, `permission_grant`; Sync #6: `document.file_id`). Reine "neue Tabelle"-Migrationen brauchen keinen 3-Step-Recovery.
- [sync7-followup] **CI-Lint-Backend Sequenz-Fixes (2026-05-04)** — Drei aufeinander folgende `fix`-Commits noetig nach dem Merge, weil Sync #7 mehrere CI-Checks neu/strenger machte und sie beim ersten Push wieder den jeweils naechsten Fehler aufdeckten:
  1. **`fix(lint): Duplicate-Key in pyproject.toml`** (Commit `181e4aa24`) — Sync #7 Auto-Merge hat zwei `[tool.ruff.lint.per-file-ignores]`-Eintraege zusammengeworfen (unser VOeB-Eintrag fuer `backend/ext/tests/**` als Inline-Table + Upstream-neuer dotted-Table fuer `backend/alembic/**`). TOML lehnt das ab, ruff failed mit `duplicate key`. Beide Inhalte zu einer einzigen `[tool.ruff.lint.per-file-ignores]`-Sektion zusammengefuehrt.
  2. **`fix(lint): I001 + G004 in backend/ext/`** (Commit `aee378233`) — 36 I001 (unsorted-imports) durch `ruff check --fix` automatisch gefixt + 10 G004 (logging-f-string) manuell auf `%`-Style umgestellt (`backend/ext/routers/doc_access.py`, `health.py`, `services/doc_access.py`, `tasks/doc_access_sync.py`). Beide Regeln sind in Sync #7 erstmals scharf gefordert — Upstream PR #10767 hat G004 global eingefuehrt, I001 wurde durch den `force-single-line`-Push schaerfer (war vorher tolerant gegen multi-line-from-modul).
  3. **`fix(format): ruff format Baseline auf backend/ext/`** (Commit `b61a3b0a6`) — 24 Files reformatiert nach `ruff format`-Default (line-length=88, double-quote, indent-space). Pre-existing Drift, monatelang in `voeb-projekt-status.md` als offener Task. Sync #7 hat ihn zur CI-Blockade gemacht: nachdem `ruff check` mit Schritt 2 gruen ist, faellt jetzt `ruff format --check` auf. Reine syntaktische Formatierung, keine Funktionsaenderung. AST-Sanity gruen.
  - **Lesson Learned:** Bei Upstream-Syncs, die neue Lint-Regeln scharf machen, **alle Lint-Stages lokal vorab durchspielen** — `ruff check` UND `ruff format --check` UND TOML-Parse. Sonst entdeckt man die Stages erst sequenziell im CI, jeder Push triggert einen Concurrency-cancelten Deploy. Drei Iterationen in 7 Min sind Verschwendung. Fuer Sync #8+ als Pflicht-Schritt im Pre-PR-Check vermerkt.

### Changed
- [upstream-sync] **Siebter Upstream-Merge auf Sync-Branch (2026-05-02)** — 168 Commits von `upstream/main` (Chart 0.4.47 → 0.4.48). Merge-Base war der Sync-#6-Stand (`5c896e2caf95`); Upstream `main` hatte vorher einen Force-Push erhalten, der den Merge-Base unangetastet liess. **5 Konflikte (alle erwartet): 2 trivial** (`AGENTS.md`, `README.md` per `git checkout --ours`) **+ 1 moderat** (`web/Dockerfile`: Upstream-Refactor mit Typo-Fix `overriden`→`overridden`, unsere drei `NEXT_PUBLIC_EXT_*`-ARG/ENV-Bloecke beibehalten) **+ 2 ernsthaft auf Core-Dateien #9 und #17**.
  - **Core #9 `web/src/components/auth/AuthFlowContainer.tsx`:** Opal-Logo-Migration (Upstream PRs #10474 + #10483). `OnyxIcon` aus `../icons/icons` (Legacy-Pfad, laut `web/CLAUDE.md` deprecated) durch `SvgOnyxLogo` aus `@opal/logos` ersetzt. Unser Custom-Logo-Conditional bleibt erhalten, der Fallback-Branch nutzt jetzt `SvgOnyxLogo`. ext-i18n-`t()`-Calls und `enterpriseSettings`-Logik unveraendert.
  - **Core #17 `web/src/sections/sidebar/AccountPopover.tsx`:** Schema-Migration `LineItem`→`LineItemButton` (Upstream PR #10646). Unser EXT_BRANDING_ENABLED-Patch (gerade 2026-04-20 gebaut) komplett auf das neue Opal-Schema (`sizePreset`, `variant`, `rounding`, `title`, `icon`, `rightChildren`) portiert. Drei Gate-Stellen aktiv geblieben (Notifications, Help & FAQ, Bubble), **eine vierte ergaenzt:** Upstream hat einen neuen "Onyx <version>"-Eintrag mit Link auf `docs.onyx.app/changelog` eingefuehrt — bei aktivem Branding-Flag ebenfalls ausgeblendet (Whitelabel-konsistent: keine externe Onyx-Referenz im VOeB-User-Menu). Bubble-Conditional auf `!!undismissedCount && !EXT_BRANDING_ENABLED` umgestellt (Upstream hat `hasNotifications` weg-refactored). Patch ist jetzt ~30 Zeilen umfassender (4 statt 3 Gate-Stellen).
  - **Auto-merged Core-Dateien (14 von 16):** alle Backend-Hooks (`main.py` Router, `multi_llm.py` Token-Hooks, `access.py` 3 ext-access-Hooks, `prompt_utils.py` ext-prompts, `persona.py` + `document_set.py` ext-rbac, `search_nlp_models.py` lowercase-Patch) und Frontend-Hooks (`layout.tsx` TranslationProvider+`lang="de"`, `constants.ts`, `LoginText.tsx`, `AdminSidebar.tsx` mit +84 Zeilen Drift, `ActionsPopover/index.tsx` Early-Return, `useSettings.ts` Branding-Gate, `DynamicMetadata.tsx` `usePathname`+`useSearchParams`) semantisch verifiziert — alle Hooks intakt.
  - **ext-Code-Anpassung (1 Datei):** `web/src/ext/components/LogoCropModal.tsx` Checkbox-Import von `@/refresh-components/inputs/Checkbox` auf `{ Checkbox } from "@opal/components"` umgestellt — Upstream hat Checkbox in das Opal-Design-System verschoben, alter Default-Export-Pfad existiert nicht mehr. `onCheckedChange` typed jetzt korrekt (`(checked: boolean) => void`). `tsc --strict` laeuft komplett sauber durch.
  - **Alembic:** 2 neue Upstream-Migrationen (`14162713706c` IndexAttempt-Stage-Metric-Tabelle, `31bd8c17325e` Targeted-Reindex-Schema). Unsere ext-Chain umgehaengt: `ff7273065d0d` (ext-branding) `down_revision` von `a7c3e2b1d4f8` auf `31bd8c17325e`. Volle Chain: `a7c3e2b1d4f8 → 14162713706c → 31bd8c17325e → ff7273065d0d → b3e4a7d91f08 → c7f2e8a3d105 → d8a1b2c3e4f5`. **3-Step-Recovery-Pattern** auf DEV-DB beim Deploy noetig (`UPDATE alembic_version = 'a7c3e2b1d4f8'` → `alembic upgrade 31bd8c17325e` → `UPDATE alembic_version = 'd8a1b2c3e4f5'`) — gleicher Pfad wie Sync #5/#6.
  - **Chart 0.4.47 → 0.4.48:** Patch-Bump, **keine neuen Sub-Charts** in `Chart.yaml` → keine CI-Repo-Ergaenzung in `stackit-deploy.yml` noetig. Alle 7 Helm-Repos unveraendert.
  - **Bemerkenswerte Upstream-Aenderungen:**
    - **2 Security-Fixes (DSGVO/Banking-relevant):** `#10602 enforce document set access in search filters` + `#10601 enforce agent access on chat session creation` + `#10528 is_listed-Filter fuer alle Nicht-Owner-Agent-Zugriffe`. Direkt relevant fuer unser RBAC-Modell (`ext-access` + `ext-rbac`).
    - **OpenSearch-Stabilitaet:** Index-Operationen hinter Redis-Lock (#10446), Index-Refresh zwischen Delete und Index (#10525, #10514) — fixt eine Race wo geloeschte Dokumente unter alten ACLs wieder auftauchen konnten.
    - **Connector-Fixes:** Confluence `/Date`-Macro + kyrillisches Encoding (#10488) — VOeB-Confluence koennte beides treffen.
    - **LLM-Stack:** `litellm 1.81.6 → 1.83.0` (#10179) — Tool-Calling-Stabilitaet auf StackIT-Modellen.
    - **Indexing-Performance:** Multi-Threading fuer Image-Processing (#10744), Cap auf Threads fuer Contextual-RAG (#10729), Default-Pruning-Frequency 14d → 20d (#10705).
    - **Tracing/Braintrust:** Neue `LLMFlow`-Registry (#10735), automatisches Wrappen jedes LLM-Calls (#10478) — laeuft passiv mit, kein Impact aber Infrastruktur fuer spaeteres Tracing vorhanden.
    - **Helm `extraEnv`-Hook (#10533):** Saubereres Mittel fuer Per-Deployment-Env-Sets — optional fuer spaetere Vespa-/Tracing-Konfiguration nutzbar.
    - **Node 20 → 24** (#10526) im Frontend-Container; **Python-Base** `python:3.11-slim-bookworm` → `python:3.11-slim` (#10741) Tag-Vereinfachung.
    - **ENV-Rename** `DANSWER_RUNNING_IN_DOCKER` → `ONYX_RUNNING_IN_DOCKER` (#10442) — wir setzen das nicht in unseren Helm-Values, kein Breaking.
    - **Vespa-Refactor** `vespa_metadata_sync_task` rename (#10613) — Vespa ist bei uns seit 2026-04-25 ueber `ONYX_DISABLE_VESPA=true` deaktiviert, Refactor unkritisch.
    - **Tooling:** `ruff` ersetzt `autoflake` und `reorder-python-imports` (#10489), `G004`-Lint scharf (alle f-Strings im Logging auf `%`-Style, #10767) — verstaerkt unseren bekannten Format-Drift in `backend/ext/`, separat zu adressieren.
  - **Backend-Tests:** Pytest fuer `backend/ext/tests/` lokal nicht ausfuehrbar (Sync-Branch hat kein aktives venv) — wird beim DEV-Deploy von CI/CD durch die `ci-checks.yml` Pipeline abgedeckt. Backend-AST-Sanity (Python-Syntax) auf allen 7 Backend-Core-Dateien + Alembic-Migration: gruen.
  - **Frontend-Tests:** `tsc --strict` 0 Errors (komplett sauber). NPM-Lint nicht zwingend, weil `tsc` die kritischen Probleme schon abdeckt.
  - **Workflow:** Sync-Branch `chore/upstream-sync-2026-05-02`, Vorbereitung abgeschlossen. Push + PR + Merge + DEV-Deploy + Alembic-Recovery offen — wartet auf Nikos Freigabe.

### Deployed
- [prod-cost] **PROD Node-Downgrade g1a.8d → g1a.4d + Vespa-PVC-Cleanup + Final-Rightsizing (2026-04-26)** — Drei aufeinander folgende Eingriffe in einer Session, alle live, alle stabil.
  - **Schritt 1 — Vespa-Cleanup:** `vespa.enabled=false` und `ONYX_DISABLE_VESPA=true` waren bereits am Vortag deployed (siehe Eintrag 2026-04-25). Heute Aufraeumen: PVC `vespa-storage-da-vespa-0` geloescht (DEV 20 GiB + PROD 50 GiB, wo nur 2,6 GiB belegt waren — Vespa-Bookkeeping ohne produktive Onyx-Daten, `total_chunks_in_vespa=0` in der DB), NetworkPolicy `03-allow-intra-namespace.yaml` Vespa-Eintraege aus Kommentaren entfernt, Monitoring-Alert `VespaStorageFull` aus `values-monitoring.yaml` + `values-monitoring-prod.yaml` raus, veraltete Doku-Kommentare in `values-dev.yaml` und `values-prod.yaml` durch Done-Hinweise ersetzt. Eingreifend war zusaetzlich `helm upgrade monitoring` auf DEV+PROD (Rev 7 jeweils) damit der Alert wirklich aus Prometheus verschwindet.
  - **Schritt 2 — PROD Node-Downgrade `g1a.8d → g1a.4d`:** `terraform apply` in `environments/prod/main.tf` mit `machine_type = "g1a.4d"`. Apply-Dauer 12m1s, 0 added, 1 changed, 0 destroyed. Nodes wurden einzeln ersetzt (Rolling), keine Pending Pods, alle 17 Onyx-Pods landen Ready. Cluster-Capacity: 2x g1a.4d = 8 vCPU + 32 GiB statt vorher 16 vCPU + 64 GiB. Live-Auslastung nach Apply: Node 1 12 % CPU / 57 % RAM, Node 2 21 % CPU / 32 % RAM. **Kostenersparnis: −283,18 EUR/Monat** (PROD Worker Nodes 566 → 283 EUR/Monat).
  - **Schritt 3 — Final Resource-Rightsizing:** Nach Werktagsdaten (Mo-Fr 30d, Sonntag explizit ausgenommen weil <0,5 % Werktagslast). Werktag-CPU-Spitze des gesamten Clusters real nur 428m avg / 586m p95 — selbst der einmalige p99-Burst (1.573m) belegte nur 20 % der Allocatable. Senkungen:
    - `values-prod.yaml`: `celery_worker_docfetching` 300m → 100m (Werktag-Peak 90m, p99 11m), `celery_worker_docprocessing` 300m → 100m (Werktag-Peak 86m, p99 11m), `opensearch` 500m → 250m (Werktag-Peak 219m, p99 13m), neuer `redisOperator`-Block (war bisher Chart-Default 500m/500Mi, Live 4m/22 Mi → Faktor 100x über; Request 100m/128Mi)
    - `values-monitoring-prod.yaml`: `prometheus` 500m → 250m (Werktag-Peak 100m)
    - `values-loki-prod.yaml`: `loki` 250m → 100m (Werktag-Peak 12m). **Loki-Helm-Conflict:** Erste `helm upgrade`-Versuche scheiterten an `kubectl-patch` Field-Manager-Konflikt auf `loki-loki-stack` ConfigMap (3-Wege-Merge), Loesung: ConfigMap geloescht, Helm legt sie beim Upgrade neu an. Eingriff bricht keine laufende Funktionalitaet (Datasource wird sofort neu generiert).
    - `redis-operator` PROD via `helm upgrade redis-operator ot-helm/redis-operator` mit `--set resources` (eigene Helm-Release, nicht Onyx-Sub-Chart).
    - `redis-operator` DEV via `kubectl patch deploy redis-operator` (Helm-Release-State verloren, Pod ist orphan; korrekter values-dev.yaml-Block existierte schon, wurde aber durch verlorene Helm-Release nicht angewendet).
    - **Cluster-Total-Requests PROD:** 7.274m → 5.824m (= 74 % der g1a.4d-Allocatable, vorher 92 %). Onyx-Komponenten-CPU-Requests: 3.400m → 2.350m. Limits unveraendert.
    - **Cluster-Total-Requests DEV:** 3.974m → 3.574m durch redis-operator-Senkung (= 48 % statt 54 %).
  - **Lessons Learned (zusaetzlich zu denen vom 2026-04-25):**
    1. **Helm-Release-State kann zwischen Cluster-Operationen verloren gehen.** DEV redis-operator-Annotations sagten "Helm-managed", aber `helm history redis-operator` lieferte "release: not found". Pod lief weiter als Orphan-Deployment, wurde direkt per `kubectl patch` reicht. Sauberer Workaround: Helm-Release neu installieren auf bestehender Workload (`helm install --replace`).
    2. **Sonntag-Snapshots sind nicht repraesentativ fuer PROD.** Werktagsmessungen (Mo-Fr per `EXTRACT(dow ...)` filtern) zeigen mittel 130-200 % der Wochenend-Last. Resource-Tuning IMMER auf Werktag-Daten basieren, sonst werden Requests zu eng.
    3. **Loki-Stack mit Chart 2.10.3 ist deprecated** und nutzt `kubectl-patch`-Owner-Annotations bei initialen `kubectl apply`-Vorgaengen, was Helm 3.13+ Server-Side-Apply blockiert. Workaround: ConfigMap manuell loeschen vor Helm-Upgrade. Langfristig: Migration auf Loki-Helm-Chart 6.x oder grafana/loki-distributed.
  - **Kostenstand nach allen heutigen Eingriffen:** DEV+TEST 585 EUR, PROD 681 EUR (war 964), Vespa-PVC-Reduktion ~5 EUR, **Total ~1.261 EUR/Monat statt 1.434 EUR/Monat** = **−173 EUR/Monat (−12 %)** als direkter Cash-Effekt. Plus implizite Effizienz: Cluster-Headroom wieder bei 26 % statt 8 %, Werktag-p99-Burst hat noch 80 %+ Reserve.

- [prod-rollout] **Sync #6 PROD + Vespa-Disable + Worker-Resource-Rebalance (2026-04-25)** — Kombinierter Rollout. Vorgang dauerte rund 4 Stunden inkl. Recovery, davon ~1 Stunde geplante Outage durch Alembic-Crash und ~1 Stunde ungeplante Outage durch eigenen Branch-Hygiene-Fehler (siehe Lessons unten). PROD-Endzustand: Helm Rev 23 `deployed`, Chart 0.4.47, Image `a211c3b`, alle 17 Pods Running, Health `/api/health` 200, alembic_version `d8a1b2c3e4f5`, ONYX_DISABLE_VESPA=true, Vespa-Pod nicht mehr existent, Worker-Resources nach 30d-Prometheus-Peaks rebalanced.
  - **Schritt 1 — DEV-Deploy (Push-to-main, Run 24925683562):** Commit `a211c3b6b` aus `feature/prod-vespa-out-and-rebalance` auf `main` gemergt. Auto-Deploy DEV ohne Probleme: Vespa-Pod terminiert (Pod-Anzahl 16 → 15), neue Worker-Resources aktiv (CPU-Requests Onyx-Komponenten 3.200m → 1.150m), Health 200, kein OOMKill, kein Pending. Soak-Voraussetzung-Daten gleich nutzbar. Cluster Node 2 RAM-Auslastung 65 % → 37 %.
  - **Schritt 2 — PROD-Trigger (workflow_dispatch, Run 24925872185):** Erwarteter Alembic-Crash. api-server in CrashLoopBackoff mit `psycopg2.errors.UndefinedColumn: column document.file_id does not exist` — Sync-#6-Code (`a211c3b`) liest `document.file_id` (eingefuehrt durch Upstream-Migration `91d150c361f6`), DB hatte aber noch `alembic_version=d8a1b2c3e4f5` ohne die 5 neuen Upstream-Migrationen. Helm Rev 21 → `failed` (context deadline exceeded). PG-Backup vor Trigger gemacht (`/tmp/prod-alembic-version-backup-20260425-093613.txt`).
  - **Schritt 3 — 3-Step-Alembic-Recovery:** Auf laufendem `celery-worker-primary`-Pod ausgefuehrt (api-server down): `UPDATE alembic_version SET version_num = '503883791c39'` → `alembic upgrade a7c3e2b1d4f8` (5 Upstream-Migrations: `d129f37b3d87 → a6fcd3d631f9 → 91d150c361f6 → 856bcbe14d79 → a7c3e2b1d4f8`) → `UPDATE alembic_version SET version_num = 'd8a1b2c3e4f5'`. Spalte `document.file_id` jetzt vorhanden, Schema konsistent. Pattern-Wiederholung Sync #5 PROD → Sync #6 PROD = 2/2.
  - **Schritt 4 — Eigene Outage durch falschen Branch (LESSON):** Manuelles `helm upgrade` aus dem Working-Directory ausgefuehrt, **ohne zu pruefen, dass der lokale Branch nicht `main` ist**. Branch war `docs/plattform-acl-praezisierung` mit alten Working-Dir-Files (vespa.enabled=true, kein ONYX_DISABLE_VESPA). Folge: Helm hat das alte Setup deployed (Vespa wieder hochgefahren) und Image-Tag auf `:latest` gesetzt (CI/CD setzt sonst Commit-SHA per `--set`). `:latest` zeigt auf Onyx-Mainline (alte Codebase) → kennt unsere ext-Migration `d8a1b2c3e4f5` nicht in seiner alembic-Chain → CrashLoopBackoff `Can't locate revision identified by 'd8a1b2c3e4f5'`. PROD-Outage ~50 Min.
  - **Schritt 5 — Recovery via Workflow-Re-Trigger (Run 24928356519):** `gh workflow run stackit-deploy.yml -f environment=prod --ref main` — checkt automatisch `main` aus, setzt korrekten Image-Tag `--set global.version=$SHA`, korrektes vespa.enabled=false. Workflow stand auf `waiting` (GitHub Environment `prod` Required Reviewer Gate), nach Approval lief er in ~5 Min durch. Helm Rev 22 → `failed` (alter Container war noch im Loop), Rev 23 → `deployed` mit korrektem Container `a211c3b`.
  - **Schritt 6 — Final Verification:** alle 17 Pods Running, Vespa-Pod NotFound, ONYX_DISABLE_VESPA=true, Health 200, Cluster-Total-Requests 7.294m CPU / 14,7 Gi RAM (frueher 8.450m / 15+ Gi). Cluster-Total ist auf 2x g1a.4d (~7.400m allocatable) bereits jetzt 99 % CPU — **Node-Downgrade muss bis zur tatsaechlichen 30d-Soak-Daten-Verifikation warten**, weil API-Peak 1.245m (einmalig) den Headroom uebersteigen koennte. Vorlaeufig 2x g1a.8d beibehalten.
  - **Lessons Learned:**
    1. **Branch-Hygiene bei manuellen Helm-Upgrades:** Vor `helm upgrade <release> -f <file>` immer `git status` + `git rev-parse HEAD` checken. Besser: nur ueber Workflow oder explicite `git checkout main && git pull` davor.
    2. **Helm-Status `failed` ist kosmetisch, aber das Heilen darf nicht aus falschem Branch erfolgen.** Sync-#6-DEV-Lesson 2 hatte das schon dokumentiert — heute live erlebt im worst case.
    3. **3-Step-Alembic-Recovery ist robust** (Sync #5 + Sync #6 = 2/2 erfolgreich). Pattern in `docs/runbooks/upstream-sync.md` validiert.
    4. **Required-Reviewer-Gate auf PROD-Workflow** verzoegert Recovery — vorab kommunizieren, dass approved werden muss. Zeitlicher Impact: Niko musste manuell zum Browser, ~3 Min Verzoegerung.
    5. **PROD ist nach Rollout NICHT direkt downgrade-fertig.** Cluster-Requests passen mathematisch auf g1a.4d, aber API-Peak (30d max 1.907 Mi RAM, 1.245m CPU) frisst zuviel Headroom bei aktiver User-Last. Erst nach 7 Tage Soak + neuer 30d-Daten-Auswertung Node-Downgrade pruefen.

- [upstream-sync] **Sechster Upstream-Merge auf DEV (2026-04-23)** — 234 Commits von `upstream/main` (Chart 0.4.44 → 0.4.47). Merge-Base `cc5a74511` (2026-04-10, Sync #5). Konflikte auf 3 Dateien beschraenkt (`.gitignore`, `AGENTS.md`, `README.md`) — alle erwartet, alle trivial aufloesbar. Die 16 gepatchten Core-Dateien wurden auto-gemergt, semantisch verifiziert (inkl. `persona.py` mit 14 Upstream-Commits Drift und `AdminSidebar.tsx` nach weiterem Opal-Refactor). **Kein neuer Core-Patch, keine ext-Code-Anpassung**, `backend/ext/auth.py`-Wrapper funktional unveraendert. Chart-Bump brachte keinen neuen Subchart, keine Helm-Repo-Ergaenzung in CI noetig. Deploy auf DEV: Helm Rev 64 (`failed`, Timeout wegen Alembic-Crash) → nach Recovery Rev 65 (`deployed`) — siehe naechster Punkt. Workflow: Branch + PR #22, Merge via `gh pr merge --admin --merge` (CI-Checks triggern nur auf push-to-main, nicht auf PR → Admin-Bypass erforderlich). 17/17 Pods `1/1 Running`, `/api/health` HTTP 200, `alembic current = d8a1b2c3e4f5 (head)`. Upstream-Drift nach Deploy: **0 Commits** — DEV auf der neuesten verfuegbaren Onyx-Version.
- [alembic] **Alembic-Chain-Recovery Sync #6 auf DEV (2026-04-23)** — Identisches Pattern zu Sync #5: unsere ext-Chain haengt am vorherigen Upstream-Head (`503883791c39`), Upstream schiebt 5 neue Migrationen dazwischen. Chain umgehaengt: `ff7273065d0d` (ext-branding) von `down_revision=503883791c39` auf `down_revision=a7c3e2b1d4f8`. Recovery auf laufender DEV-DB erforderlich, weil unsere ext-Tabellen bereits seit Monaten existieren und die Upstream-Migrationen dazwischen live auf DEV noch nicht gelaufen waren. **Der Crash-Trigger:** Die Upstream-Migration `91d150c361f6_add_file_id_to_documents` fuegt eine `file_id`-Spalte zur `document`-Tabelle hinzu, auf die der neue Code sofort beim Boot zugreift (`SELECT document.file_id FROM document`). Ohne gelaufene Migration: `psycopg2.errors.UndefinedColumn: column document.file_id does not exist` → `Application startup failed. Exiting` → CrashLoopBackOff nach 6 Restarts. Recovery-Sequenz (dokumentiertes 3-Step-Pattern, auf einem Celery-Pod ausgefuehrt da api-server down war): `UPDATE alembic_version = '503883791c39'` → `alembic upgrade a7c3e2b1d4f8` (5 Migrations: `d129f37b3d87 → a6fcd3d631f9 → 91d150c361f6 → 856bcbe14d79 → a7c3e2b1d4f8`) → `UPDATE alembic_version = 'd8a1b2c3e4f5'` (zurueck auf unseren Head, da ext-Migrationen bereits gelaufen). API-Pod-Restart, Helm erkannte Ready, Release wechselte in `deployed`. **Fuer PROD-Rollout beibehalten** — gleiche 3 SQL-Statements, gleiche Reihenfolge.

### Fixed
- [ext] **CI-Lint-Backend: Unused Imports + F821/ARG001 (2026-04-23)** — Pre-existing Lint-Drift, nicht durch Sync #6 verursacht (die Imports waren seit mehreren Modulen unused, aber `ruff check` auf DEV lief nie gruen — vermutlich weil die ext/-Lint-Stage in `ci-checks.yml` erst kuerzlich hinzugekam). 12 Unused Imports entfernt via `ruff check --fix` (mehrere `datetime.*`, `ext.services.prompt_manager.get_prompt_by_id`, `onyx.access.models.DocumentAccess`, `sqlalchemy.orm.joinedload`, `time`, `pytest`). 3x F821 Undefined-Name (quoted forward references `ExtCustomPrompt`, `PromptCreate`, `PromptUpdate`) durch `TYPE_CHECKING`-Imports geloest, die bisherigen `# type: ignore[name-defined]`-Kommentare konnten entfernt werden. 2x ARG001 (unused `self` bei Celery `bind=True`-Tasks in `audit_ip_anonymize.py` + `doc_access_sync.py`) mit `# noqa: ARG001` annotiert — `self` ist von Celery required und nicht entfernbar. Commit `d73d2353b` auf Feature-Branch, lokaler Merge auf main + Admin-Bypass-Push. **Offen:** `ruff format --check backend/ext/` meldet noch 28 Files Drift (pre-existing, ~640/-, ~443/+ Zeilen-Umbau multi-line → single-line). Separater Task — Voraussetzung ist explizite Ruff-Config in `pyproject.toml` (line-length, quote-style), damit das Format deterministisch wird und nicht ohne Vorwarnung driftet.
- [helm-prod] **celery_worker_primary auf Singleton reduziert (2026-04-23)** — `replicaCount` in `values-prod.yaml` von 2 auf 1. Die bisherige Annahme "HA via 2 Replicas" war technisch unrichtig: Onyx nutzt einen Redis-basierten Primary-Lock mit exklusiver Aktivierung. Ein zweiter Pod wuerde nur im Standby auf den Lock warten und keinerlei Arbeit uebernehmen (kein Active-Active, kein Load-Balancing). Recovery-Pfad bei Pod-Crash: K8s Deployment startet in <30s einen neuen Pod, der neue Pod erwirbt den Redis-Lock beim Boot, kein persistenter Write-Conflict. Der bisherige Kommentar im Values-File war irrefuehrend; ein inaktiver Standby-Pod blockiert kontinuierlich Ressourcen ohne Nutzen. PROD-Deploy via `workflow_dispatch` separat einzuplanen. Nur PROD betroffen — DEV hatte schon `replicaCount: 1`.

### Removed
- [infra] **TEST-Live-Infrastruktur vollstaendig abgebaut (2026-04-21)** — StackIT-Ressourcen geloescht: Helm Release `onyx-test` uninstalled (LoadBalancer-IP 188.34.118.201 freigegeben), Namespace `onyx-test` geloescht, PostgreSQL Flex `vob-test` (Instance `d371f38d-…`) per StackIT CLI geloescht (inkl. 30-Tage-Backups, unwiderruflich), Object Storage Bucket `vob-test` geleert (4 Objekte) und geloescht. Terraform-State via `terraform state rm` fuer 4 Ressourcen bereinigt. **Einsparung: ~115 EUR/Monat** (PG Flex 2.4 + Load Balancer + Bucket). Haupt-Compute war bereits seit 2026-03-19 auf 0 Pods heruntergefahren, aber Managed Services liefen weiter und erzeugten vermeidbare Kosten. Offene Nacharbeit: DNS-Eintrag `test.chatbot.voeb-service.de` bei GlobVill + Cloudflare aufraeumen (Leif).

### Changed
- [infra] **TEST als Template-Blueprint im Repo erhalten (2026-04-21)** — Nach dem Live-Abbau bleiben die Konfigurations-Artefakte als Blueprint fuer zukuenftige Kunden-Klon-Projekte und fuer eventuelle Reaktivierung einer TEST-Umgebung: `deployment/terraform/environments/test/main.tf` (mit 5-Schritt-Reaktivierungsanleitung im Header), `deployment/helm/values/values-test.yaml` (Template-Marker), `.github/workflows/stackit-deploy.yml` `deploy-test`-Job (Kommentar: "INAKTIV, nur via workflow_dispatch"). Modul `deployment/terraform/modules/stackit-data/` bleibt mit `prevent_destroy = true` erhalten (wiederverwendbar fuer Environments ohne eigenen Cluster). `docs/referenz/technische-parameter.md` Tabellen auf DEV/PROD reduziert; `.claude/rules/voeb-projekt-status.md` TEST-Abschnitt umformuliert.

### Changed
- [ext-branding] **User-Menu aufgeraeumt: Notifications + Help & FAQ ausgeblendet (2026-04-20)** — Core #17 `web/src/sections/sidebar/AccountPopover.tsx` neu eingerichtet. Beim Klick auf den eigenen Namen (unten links in der Sidebar) oeffnet sich ein Popover-Menu. Im Upstream-Stand enthielt es vier Eintraege: "User Settings" (→ `/app/settings`), "Notifications (N)" (Onyx Release-Notes/Announcements), "Help & FAQ" (→ `docs.onyx.app`), Trenner, "Log out". VOeB nutzt weder das Onyx-interne Announcement-System noch soll die Oberflaeche auf `docs.onyx.app` verweisen (Whitelabel-Anforderung — externe Onyx-Referenz untergraebt die VOeB-Markenfuehrung). Fix: Neue Konstante `EXT_BRANDING_ENABLED` (gleicher Flag wie Core #15) + `!EXT_BRANDING_ENABLED &&`-Gate an drei Stellen: (1) Notifications-Menu-Item, (2) Help-&-FAQ-Menu-Item, (3) roter Notifications-Bubble-Indikator am Sidebar-Trigger (sonst leuchtet er sinnlos ohne anklickbaren Kontext). Ergebnis: Menu zeigt nur noch "Benutzereinstellungen" → Trenner → "Abmelden". SWR-Fetches auf `/api/notifications` bleiben erhalten, damit ein Rueckzug via Flag-off voll funktioniert (Upstream-Behaviour respektiert). Doku-Update: `core-dateien.md` (Core #17 neu), `fork-management.md` (Merge-Stellen-Tabelle + 16→17), `extension-regeln.md`/`enterprise-grundsaetze.md`/`CLAUDE.md` (alle Zaehler 16→17), Pre-commit Hook Whitelist erweitert. Patch ist ~30 Zeilen (11 Zeilen Konstante+Kommentar, 2x7 Zeilen Array-Gate, 1 Zeile Bubble-Conditional).

### Fixed
- [ext-branding] **404-Retry-Loop auf `/api/enterprise-settings/custom-analytics-script` gestoppt (2026-04-20)** — Core #15 `web/src/hooks/useSettings.ts` reduziert. Der urspruengliche Sync-#5-Patch gated neben `useEnterpriseSettings()` AUCH `useCustomAnalyticsScript()` hinter `NEXT_PUBLIC_EXT_BRANDING_ENABLED`. Folge in PROD: SWR triggerte den Fetch auf einen Endpoint, der ausschliesslich in `backend/ee/` (EE-only) existiert und in unserem FOSS-Fork 404 zurueckgibt. Mit SWR-Default `shouldRetryOnError: true` entstand ein Endlos-Loop aus `setTimeout`-Retries (Exponential-Backoff), der die Browser-Konsole mit mehreren Hundert 404-Meldungen pro Minute zuspammte. Fix: Gate auf Upstream-Original zurueck (`EE_ENABLED || eeEnabledRuntime`). `useEnterpriseSettings()` bleibt mit `EXT_BRANDING_ENABLED` erweitert — nur der Custom-Analytics-Hook wird nicht mehr gated, weil VOeB kein Custom Analytics Script nutzt. `core-dateien.md` Core #15 Beschreibung angepasst (ERLAUBT nur noch 1 shouldFetch-Stelle, VERBOTEN expliziter Warnhinweis gegen erneutes Gaten), Patch-Datei `backend/ext/_core_originals/useSettings.ts.patch` regeneriert. Bei zukuenftigem Bedarf nach Custom-Analytics-Support: eigener `ext-analytics`-Endpoint statt EE-Pfad.
- [helm] **Rate-Limit 429-Spam beim Chat-Sidebar-Rendering behoben (2026-04-20)** — NGINX SEC-09 Rate-Limiting auf `/api/*` gescopt. Vorher galten `rate=10r/s` + `burst=50 nodelay` global (server-weit), was Next.js App Router beim Sidebar-Render sofort zur Last wurde: Der RSC-Prefetch laedt fuer jeden sichtbaren Chat-Link `/app?chatId=xxx&_rsc=seere` parallel vor — bei 20+ Chats ueberfuellt das den Burst, Rest wird mit HTTP 429 abgelehnt. Konsole zeigte 429-Salven auf harmlosen Seiten-Navigationen. Fix: `map $uri $ratelimit_key { default ""; "~^/api/" $binary_remote_addr; }` im `http-snippet`, `limit_req_zone` auf den gemappten Key. NGINX-Verhalten: Leere Keys werden von `limit_req_zone` nicht gezaehlt → nur API-Pfade zaehlen, Seiten-Requests passieren ungehindert. Der DoS- und LLM-Kosten-Schutz bleibt auf der gefaehrdeten Oberflaeche voll erhalten. In `values-common.yaml` (DEV/TEST) und `values-prod.yaml` synchron eingetragen (Helm Deep Merge ersetzt `nginx.controller.config` komplett, Config muss in beiden Dateien stehen). SEC-09 im Sicherheitskonzept entsprechend ergaenzt.
