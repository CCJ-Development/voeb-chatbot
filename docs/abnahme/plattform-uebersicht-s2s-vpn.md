...
> **Netzwerk-Zugang ausschließlich über VÖB-Netz:** Der Chatbot ist in dieser Architektur nur über einen IPsec-Site-to-Site-VPN-Tunnel zwischen dem VÖB-Rechenzentrum und der StackIT-Cloud erreichbar. Es gibt keinen öffentlich erreichbaren HTTPS-Endpunkt. Mitarbeiter greifen direkt aus dem VÖB-Büro oder über das VÖB-Corporate-VPN auf den Chatbot zu.
...
**Netzwerk-Zugang über Site-to-Site-VPN:** Der Chatbot ist über einen **IPsec-Tunnel zwischen VÖB-Rechenzentrum und StackIT-VPC** angebunden. Der Dienst hat keinen öffentlich erreichbaren HTTPS-Endpunkt. Nutzer-Traffic fließt ausschließlich durch den Tunnel; Port-Scans aus dem Internet finden den Chatbot nicht. Erreichbar ist er nur für Mitarbeiter, die sich entweder direkt im VÖB-Netz befinden oder über das VÖB-Corporate-VPN (Global Protect o. ä.) in das VÖB-Netz verbunden haben. Der Tunnel wird über eine **Palo Alto VM-Series HA-Instanz** im StackIT-VPC terminiert — die gleiche Firewall-Plattform, die VÖB bereits im eigenen Rechenzentrum einsetzt.

Alle Nutzdaten (Chat-Konversationen, hochgeladene Dokumente, Audit-Logs) bleiben in der StackIT-Region EU01 (Deutschland); es findet kein Datentransfer in Drittländer statt. Die verwendeten Sprachmodelle (GPT-OSS 120B, Qwen3-VL 235B, Llama 3.3 70B) laufen ebenfalls auf StackIT-Infrastruktur unter vertraglich zugesicherter "Zero Data Retention" — Prompts und Antworten werden nicht gespeichert und nicht für Modelltraining verwendet. Die Authentifizierung erfolgt über **Microsoft Entra ID (OIDC)**, wodurch die bestehenden Identitäts- und MFA-Regeln der VÖB direkt greifen. Der OIDC-Flow ist mit dem Tunnel voll kompatibel: Der Login-Flow besteht aus Browser-Redirects, Entra ID muss den Chatbot nie direkt erreichen.
...
**Zusätzlich: Netzwerk-Schicht** — IPsec-Site-to-Site-VPN zwischen VÖB-Rechenzentrum und StackIT-VPC, terminiert auf beiden Seiten durch Palo Alto VM-Series Firewalls im Active/Passive-HA-Modus.

```mermaid
flowchart TB
    subgraph VOEB["VÖB-Rechenzentrum"]
        BROWSER[VÖB-Mitarbeiter<br/>Browser im Büro<br/>oder via Corporate-VPN]
        VOEBFW[VÖB Palo Alto HA<br/>IPsec-Endpunkt<br/>bestehendes Setup]
    end

    subgraph CF["Cloudflare DNS (nur Namensauflösung)"]
        DNS[chatbot.voeb-service.de<br/>löst auf private IP im StackIT-VPC]
    end

    subgraph STACKIT["StackIT EU01 Deutschland (BSI C5 Typ 2)"]
        subgraph VPC["StackIT VPC — Untrust-Subnetz"]
            PA1[Palo Alto VM-Series<br/>Primary]
            PA2[Palo Alto VM-Series<br/>Secondary]
            VIP[VIP<br/>Floating IP im Untrust]
            PA1 <-.->|HA-Sync PANOS| PA2
            PA1 -.-> VIP
            PA2 -.-> VIP
        end

        subgraph PROD["Kubernetes-Cluster vob-prod — Trust-Subnetz"]
            LB[NGINX Ingress internal<br/>TLS 1.3 ECDSA P-384<br/>HSTS 1 Jahr<br/>private IP im VPC<br/>Rate-Limit 10 r/s<br/>Upload-Limit 20 MB]
    ...
    subgraph EXT_IDP["Externe Dienste — nur Browser-Flows + Backend-Outbound"]
        ENTRA[Microsoft Entra ID<br/>OIDC Authorization Code<br/>login.microsoftonline.com]
        LE[Let's Encrypt<br/>ACME DNS-01 via Cloudflare]
    end

    subgraph GH["GitHub (CI/CD)"]
        RUNNER[Self-hosted Runner<br/>im VÖB-Netz<br/>Deploy via Tunnel]
    end

    BROWSER -->|HTTPS, DNS-Lookup| DNS
    BROWSER -->|HTTPS| VOEBFW
    VOEBFW <==>|IPsec-Tunnel über Internet| VIP
    VIP --> LB
    LB --> WEB --> API
    API <--> PG
    API <--> RD
    API <--> OS
    API <--> S3
    API -->|LLM über StackIT-Netz| AI
    API <-->|OIDC-Token-Austausch outbound| ENTRA
    CEL <--> PG
    CEL <--> OS
    CEL <--> S3
    CEL -->|Indexing| AI
    BROWSER <-.->|OIDC-Login-Flow via Browser| ENTRA
    CERT <-->|ACME| LE
    PROM -.->|Scrape| API
    PROM -.->|Scrape| PG
    PROM -.->|Scrape| OS
    BLACK -.->|Probe intern| LB
    RUNNER -->|Helm via Tunnel| PROD
    RUNNER --> REG
```

**Anmerkung zum Diagramm:**
- Die Doppel-Pfeile `<==>` markieren den IPsec-Tunnel als kritische Netzwerk-Grenze. Sämtlicher Nutzer-Traffic zum Chatbot läuft ausschließlich durch diesen Tunnel. Auf beiden Seiten terminiert er auf einer Palo Alto HA-Instanz — VÖB-seitig auf der bestehenden Palo-Alto-Infrastruktur, StackIT-seitig auf einem Palo-Alto-VM-Series-Paar im Untrust-Subnetz der StackIT-VPC.
- Der OIDC-Login besteht aus **Browser-Redirects** (Nutzer-Browser ↔ Microsoft Entra ID) und einem **Backend-Outbound-Call** für den Token-Austausch (Chatbot ↔ Entra ID). Entra ID muss den Chatbot nie inbound erreichen; der Chatbot nutzt seinen normalen StackIT-Outbound-Pfad für den Token-Austausch.
- Let's Encrypt erreicht ausschließlich Cloudflare (DNS-01-Challenge) — nicht den Chatbot. Das Zertifikat ist für die Domain gültig, nicht für die IP.
- Für den Deploy-Prozess läuft ein Self-hosted GitHub Actions Runner im VÖB-Netz. Er erreicht den Kubernetes-API-Server über denselben Tunnel wie die Nutzer.
...
| Ingress / Reverse-Proxy | NGINX Ingress Controller (internal) | — | TLS-Terminierung, Security-Header, Rate-Limit; private IP im StackIT-VPC |
| Site-to-Site-VPN (StackIT-Seite) | Palo Alto VM-Series HA (Active/Passive) auf StackIT IaaS | PAN-OS aktuell | IPsec-Terminierung, Routing, Security-Policies |
| Site-to-Site-VPN (VÖB-Seite) | bestehende VÖB Palo Alto HA-Infrastruktur | PAN-OS VÖB-seitig | IPsec-Terminierung am VÖB-Edge |
| CI/CD-Runner | GitHub Actions, Self-hosted Runner im VÖB-Netz | — | Deploy zum K8s-API-Server über den Tunnel |
| TLS-Zertifikate | cert-manager + Let's Encrypt | v1.19.4 | ECDSA P-384, TLSv1.3, Auto-Renewal über DNS-01 |
...
| VPC mit eigenen Subnetzen | Untrust (Firewall-Pair + VIP), Trust (SKE-Cluster + Managed Services), Management (Firewall-Admin) |
| Compute (IaaS) | 2× VM für Palo Alto VM-Series (Active/Passive-HA) |
...
### 4.4 Site-to-Site-VPN-Anbindung über Palo Alto HA

Der Chatbot ist ausschließlich über einen privaten IPsec-Tunnel zwischen dem VÖB-Rechenzentrum und der StackIT-VPC erreichbar. Der Tunnel wird auf beiden Seiten durch **Palo Alto VM-Series Firewalls** terminiert — die Plattform, die VÖB bereits im eigenen Rechenzentrum einsetzt. Auf StackIT-Seite läuft ein Active/Passive-HA-Paar als virtuelle Appliances im VPC.

Die Referenz-Architektur dafür wird von StackIT Professional Service als Terraform-Modul bereitgestellt (`terraform-paloalto-ha`); das Deployment-Modell ist dokumentiert und erprobt.

#### Palo Alto HA-Setup auf StackIT

| Komponente | Beschreibung |
|---|---|
| Anzahl VMs | 2× Palo Alto VM-Series, Active/Passive |
| VM-Dimensionierung | klein, passend für ≤ 200 Mbit/s interner Chatbot-Traffic (2–4 vCPU, 8–16 GB RAM je VM) |
| HA-Mechanismus | PANOS-native Active/Passive mit Preemption — Failover via VIP + Gratuitous ARP |
| VIP-Konfiguration | `/32`-Adresse als Floating-IP in `allowed_address_pairs` auf beiden Firewall-Interfaces (StackIT-VPC-Primitive) |
| Port Security | aktiviert auf allen VIP-tragenden Interfaces |
| HA-Sync | PANOS-interner Sync-Link im dedizierten HA-Subnetz |
| Lizenzierung | BYOL über bestehendes VÖB-Palo-Alto-Enterprise-Agreement — keine zusätzlichen Lizenzkosten |

**Aufteilung der Verantwortlichkeit:**

| Aufgabe | Verantwortlich |
|---|---|
| StackIT-VPC-Infrastruktur (Subnetze, Security Groups, VM-Instanzen) | CCJ Development (Terraform via StackIT-Referenz) |
| Palo Alto VM-Installation und Basis-Config | VÖB-Netzwerk-Team (Palo-Alto-Expertise vorhanden) |
| IPsec-Tunnel-Konfiguration (PANOS-Policies, Peers, Routing) | VÖB-Netzwerk-Team — auf beiden Seiten konsistent, da gleiche Plattform |
| Security-Policies auf der Cloud-Firewall | VÖB-Netzwerk-Team nach Abstimmung mit CCJ |
| Tunnel-Monitoring (Prometheus-Integration) | CCJ Development |

#### Subnetz-Layout in der StackIT-VPC

| Subnetz | Zweck |
|---|---|
| Untrust | VIP-Interface der Palo Alto (Richtung Internet/VÖB-Tunnel); Floating IP liegt hier |
| Trust | SKE-Cluster `vob-prod`, interner LoadBalancer für NGINX Ingress |
| Management | Admin-Zugang zu den Palo Alto VMs (nur VÖB-Admins, ACL-beschränkt) |
| HA-Sync | dedizierter Link zwischen Primary und Secondary |

#### Tunnel-Parameter (Richtwerte)

| Parameter | Wert |
|---|---|
| Protokoll | IPsec IKEv2 |
| Verschlüsselung Phase 1 | AES-256-GCM, SHA-384, DH-Group 20 (ECP-384) |
| Verschlüsselung Phase 2 | AES-256-GCM, Perfect Forward Secrecy |
| Re-Keying-Intervall | Phase 1: 24 h, Phase 2: 1 h |
| Dead-Peer-Detection | aktiv, 30 Sekunden |

Die hier genannten Werte sind BSI-TR-02102-3-konform und entsprechen dem Standard-Profil, das VÖB bereits zwischen bestehenden Standorten einsetzt.

#### Routing-Konzept

| Richtung | Beschreibung |
|---|---|
| VÖB-Netz → StackIT-VPC | Nutzer-Traffic zum Chatbot-Ingress und CI/CD-Deploys zum K8s-API-Server |
| StackIT-VPC → VÖB-Netz | kein Traffic — der Chatbot ruft aktiv nichts im VÖB-Netz auf |
| StackIT-VPC → Internet (outbound, nicht durch Tunnel) | für Entra-ID-Token-Austausch, StackIT AI Model Serving, Cloudflare DNS, Image-Pulls — läuft über den StackIT-Internet-Egress, nicht über den Tunnel |

Der Tunnel transportiert ausschließlich **Nutzer- und Admin-Traffic Richtung Chatbot** — kein Reverse-Traffic.

#### DNS-Verhalten

`chatbot.voeb-service.de` bleibt als Name erhalten. Zwei Varianten sind gängig — die Entscheidung trifft das VÖB-Netzwerk-Team:

- **Public DNS zeigt auf private VPC-IP:** Ein Angreifer bekommt die IP zwar per DNS-Lookup, kann sie aber aus dem Internet nicht routen. Einfachste Lösung.
- **Split-Horizon-DNS:** VÖB-internes DNS löst den Hostnamen auf die private IP auf; im öffentlichen DNS existiert kein A-Record. Saubere Trennung, etwas mehr Aufwand in der VÖB-DNS-Infrastruktur.

#### Monitoring des Tunnels

| Metrik | Quelle | Alert-Schwelle |
|---|---|---|
| Tunnel-Status (Phase 1 + Phase 2 up) | Palo Alto SNMP / syslog → Prometheus | bei Ausfall > 2 Min |
| HA-Failover-Events | Palo Alto HA-Log | bei jedem Failover informativ |
| Durchsatz / Bandbreite | Palo Alto Metriken | bei ≥ 80 % konfigurierter Linkbreite |
| Verfügbarkeit Chatbot aus VÖB-Netz | interner Blackbox Exporter | bei Fehlern > 3 aufeinander |
...
> **Zusammenspiel mit dem Site-to-Site-VPN:** Der OIDC-Flow ist vollständig kompatibel mit dem VPN-Setup. Er besteht aus zwei Teilen: (1) **Browser-Redirects** zwischen dem Mitarbeiter-Browser (im VÖB-Netz) und Microsoft Entra ID — diese laufen über das öffentliche Internet via VÖB-Perimeter, wie jeder andere Microsoft-365-Traffic auch. (2) **Ein Backend-Outbound-Call** vom Chatbot nach Microsoft, um den Authorization-Code gegen die Tokens zu tauschen — dieser läuft über den StackIT-Internet-Egress, nicht über den Tunnel. Entra ID muss den Chatbot zu keinem Zeitpunkt inbound erreichen. Der Flow ist also auch ohne öffentlich erreichbaren Chatbot-Endpunkt voll funktionsfähig.
...
graph LR
    subgraph L1["Layer 1: Perimeter"]
        A1[Site-to-Site-VPN<br/>IPsec VÖB ↔ StackIT<br/>kein öffentlicher Endpunkt<br/>Cloudflare nur DNS + ACME]
    end
    subgraph L2["Layer 2: Transport"]
        A2[NGINX Ingress internal<br/>TLS 1.3 ECDSA P-384<br/>HSTS 1 Jahr<br/>Rate-Limit 10 r/s<br/>Upload-Limit 20 MB]
    end
...
### Layer 1 — Perimeter (Site-to-Site-VPN über Palo Alto HA)

Der Chatbot hat **keinen öffentlich erreichbaren HTTPS-Endpunkt**. Sämtlicher Nutzer-Traffic fließt durch einen IPsec-Tunnel zwischen der VÖB-Palo-Alto-Infrastruktur im VÖB-Rechenzentrum und einem Palo-Alto-HA-Paar im StackIT-VPC. Aus dem öffentlichen Internet ist der Chatbot-Service schlicht nicht sichtbar — Port-Scans auf der Domain finden keinen offenen Port, Angriffsflächen wie Credential-Stuffing oder DDoS laufen am Zieldienst vorbei.

Cloudflare bleibt in reduzierter Rolle erhalten: **nur noch Namensauflösung** (DNS-only) und **Let's-Encrypt-DNS-01-Challenge** für die automatische Zertifikatserneuerung. Es gibt keine TLS-Terminierung bei Cloudflare; Cloudflare sieht nie verschlüsselten Nutzer-Traffic. Die DNS-Einträge zeigen auf die private IP im StackIT-VPC — eine Auflösung aus dem Internet läuft ins Leere, aus dem VÖB-Netz wird sie korrekt über den Tunnel geroutet.

**Praktische Konsequenz:** Ein Mitarbeiter ohne VÖB-Netz-Anbindung (z. B. von privatem WLAN ohne Corporate-VPN oder von einem nicht-verwalteten Endgerät) kann den Chatbot **gar nicht erst sehen** — unabhängig davon, ob er gültige Entra-Anmeldedaten besitzt. Das ist das zentrale Schutzversprechen dieser Netzarchitektur.

### Layer 2 — Transport (NGINX Ingress, intern)

- NGINX Ingress mit privater IP (nicht öffentlich erreichbar — nur im StackIT-VPC sichtbar)
- **TLS 1.3 only** mit Let's-Encrypt-Zertifikat, ECDSA P-384 (BSI-TR-02102-2-konform) — Zertifikat wird über DNS-01-Challenge ausgestellt und ist für die Domain gültig, nicht für die IP
...
- Echte Client-IP durch `externalTrafficPolicy: Local` — Die Client-IP ist hier die tatsächliche VÖB-interne IP des Mitarbeiter-Arbeitsplatzes, nicht eine NAT-IP des öffentlichen Internets
...
| VPC-Infrastruktur (Subnetze, Security Groups, Floating IP) | **Ja** (IaaS-Primitive) | — | Konfiguration per Terraform (StackIT-Referenz-Modul) |
| Palo Alto HA-Paar im StackIT-VPC | VM-Hosting (IaaS) | — | VM-Provisioning durch CCJ, PAN-OS-Konfiguration durch **VÖB-Netzwerk-Team** |
| IPsec-Tunnel-Config (PAN-OS-Policies, Peers, Routing) | — | — | **VÖB-Netzwerk-Team** (beidseitig konsistente Config) |
| Interner LoadBalancer (kein Public-Exposure) | **Ja** (nativer OpenStack-Support) | — | IngressClass-Konfiguration |
...
flowchart LR
    subgraph TB0["Trust Boundary 0: Internet"]
        INET[öffentliches Internet<br/>für Chatbot-Traffic NICHT traversiert]
    end

    subgraph TBV["Trust Boundary V: VÖB-Netz"]
        USER[VÖB-Mitarbeiter<br/>Arbeitsrechner]
        VOEBFW[VÖB Palo Alto HA<br/>IPsec-Endpunkt VÖB-Seite]
    end

    subgraph TB1["Trust Boundary 1: StackIT EU01 (BSI C5)"]
        VPNGW[Palo Alto HA<br/>IPsec-Endpunkt StackIT-Seite]

        subgraph TB2["Trust Boundary 2: NGINX Ingress internal (TLS-Terminierung)"]
            NGINX[NGINX Rate-Limit + HSTS + Security-Headers<br/>private IP im VPC]
        end
    ...
    subgraph TB6["Trust Boundary 6: Externe IdP"]
        ENTRA_EXT[Microsoft Entra ID<br/>nur Browser-Redirects]
    end

    USER -->|HTTPS im VÖB-Netz| VOEBFW
    VOEBFW ==>|IPsec-Tunnel| VPNGW
    VPNGW --> NGINX
    NGINX -->|intra-cluster| AUTH
    USER <-.->|Browser-Redirects<br/>durchquert VÖB-Perimeter| ENTRA_EXT
    AUTH --> RAG
    AUTH --> TOOL
...
**Externe Verbindungen** beschränken sich auf wenige klar umrissene Fälle. Keine davon erfordert, dass ein externer Dienst den Chatbot inbound erreicht — der Chatbot ist aus dem Internet unsichtbar.

1. **Microsoft Entra ID** — Der OIDC-Login besteht aus Browser-Redirects (Nutzer ↔ Microsoft) plus einem Backend-Outbound-Call vom Chatbot zu Microsoft für den Token-Austausch. Entra ID ruft den Chatbot nie auf.
2. **Let's Encrypt** — automatische Erneuerung der TLS-Zertifikate via DNS-01-Challenge. Kein HTTP-Traffic zum Chatbot nötig — die Challenge wird über einen DNS-Eintrag bewiesen.
3. **Cloudflare DNS** — reine Namensauflösung, kein Proxy, kein Traffic-Pfad zum Chatbot.
4. **StackIT AI Model Serving** — LLM- und Embedding-Aufrufe des Chatbots laufen StackIT-intern innerhalb EU01, ohne Internet-Transit.

**Nutzer-Traffic verlässt nie das gekapselte VÖB↔StackIT-Netz.** Service-seitiger Outbound-Traffic (OIDC-Token-Austausch, ACME, DNS, Image-Pulls, LLM) bleibt technisch nötig, aber er ist klar definiert und läuft nicht über den Tunnel. Weder der Prompt noch der vom Chatbot gelesene RAG-Kontext werden an Dritte übertragen.
...
- **Nutzer-Traffic (Browser ↔ Chatbot):** ausschließlich durch den IPsec-Tunnel zwischen VÖB-Palo-Alto und StackIT-Palo-Alto-HA. Das öffentliche Internet wird nie traversiert.
- **LLM-Traffic (Chatbot ↔ StackIT AI Model Serving):** StackIT-intern innerhalb der EU01-Region, ohne Internet-Transit.
- **Service-Outbound (Chatbot → Microsoft-Token-Endpoint, ACME, Cloudflare-API, Image-Pulls):** läuft über den StackIT-Internet-Egress, nicht über den Tunnel — der Chatbot wird dabei aber nie von außen angesprochen.
...
| Runner-Typ | **Self-hosted Runner im VÖB-Netz** — der K8s-API-Server ist nur über den Tunnel erreichbar, daher erfolgt das Deploy durch einen VÖB-seitigen Runner. |
...
| Smoke-Test PROD | läuft cluster-intern via Blackbox Exporter (die private IP ist nur aus dem Tunnel erreichbar) |
...
**Self-hosted Runner — Betriebsmodell:**

| Komponente | Ort | Aufgabe |
|---|---|---|
| Self-hosted GitHub Actions Runner | kleine VM im VÖB-Netz (2 vCPU, 4 GB RAM ausreichend) | `helm upgrade`, `kubectl`, `terraform apply` gegen StackIT über den Tunnel |
| Runner-Registrierung | GitHub Repo Settings → Actions → Runners | Runner wird über GitHub authentifiziert, keine inbound-Verbindung nötig |

Der Runner baut eine **outbound-Verbindung** zu GitHub auf (long-poll), zieht neue Jobs herunter und führt sie im VÖB-Netz aus. Es wird keine inbound-Verbindung von GitHub ins VÖB-Netz benötigt — das passt zu bestehenden VÖB-Firewall-Regeln.
...
| Netzwerk-Team | **VÖB Service** — Ansprechpartner für Palo Alto-Konfiguration (beidseitig), IPsec-Tunnel, Self-hosted-Runner-VM |
...
| Firewall-Plattform | **Palo Alto Networks** — VM-Series im StackIT-VPC (BYOL über bestehendes VÖB-Enterprise-Agreement) |
...
## 26. Umsetzung und Inbetriebnahme

Die Einführung der VPN-Anbindung erfolgt in einem klar abgegrenzten Umsetzungsfenster — parallel zum bestehenden PROD-Betrieb (der heute ohne Tunnel läuft). Das Umsetzungsfenster umfasst Infrastruktur-Aufbau, Firewall-Konfiguration und eine finale Umschaltung, bei der der Chatbot vom öffentlichen Endpunkt auf den Tunnel-only-Endpunkt migriert wird.

### 26.1 Arbeitsschritte

| Aufgabe | Verantwortlich | Dauer |
|---|---|---|
| VPC-Subnetz-Layout (Untrust / Trust / Management) per Terraform bereitstellen | CCJ Development (StackIT-Referenz-Modul `terraform-paloalto-ha`) | 1–2 Tage |
| 2× Palo Alto VM-Series im Untrust-Subnetz provisionieren (Active/Passive-HA mit VIP) | CCJ Development | 1 Tag |
| PAN-OS-Basiskonfiguration auf beiden Firewall-VMs (Interfaces, Zones, HA-Sync) | VÖB-Netzwerk-Team | 1–2 Tage |
| IPsec-Tunnel-Konfiguration (Peers, Policies, Routing) auf VÖB- und StackIT-Seite | VÖB-Netzwerk-Team | 1–2 Tage |
| Cluster-Ingress auf internen LoadBalancer umstellen, DNS auf private IP umziehen | CCJ Development | 0,5–1 Tag |
| Self-hosted GitHub Actions Runner im VÖB-Netz einrichten | VÖB IT + CCJ Development | 0,5 Tage |
| Tunnel-Monitoring in Prometheus einbinden (SNMP/Syslog-Integration) | CCJ Development | 0,5–1 Tag |
| End-to-End-Funktionstest (Login, Chat, Dokument-Upload, Admin-Funktionen) | CCJ Development + VÖB | 1 Tag |
| **Summe** | | **ca. 6–9 Arbeitstage** |

### 26.2 Zeitliche Planung

- **Technischer Gesamtaufwand:** ca. 6–9 Arbeitstage, auf CCJ Development und VÖB-Netzwerk-Team verteilt.
- **Kalenderzeit:** ca. 2–3 Wochen für die geordnete Umsetzung mit Abstimmungsrunden und Prüfschritten.
- **Betriebliche Nebenwirkung:** Während der Umsetzungsphase bleibt der aktuelle öffentliche Endpunkt verfügbar. Die Umschaltung auf den Tunnel-only-Betrieb erfolgt in einem kurzen, angekündigten Wartungsfenster (ca. 30 Minuten).

### 26.3 Voraussetzungen auf VÖB-Seite

- **Palo Alto Enterprise-Agreement** ist vorhanden (Annahme — zu bestätigen). BYOL auf den Cloud-VM-Series ist damit abgedeckt.
- **VÖB-seitige Firewall** kann einen zusätzlichen IPsec-Tunnel terminieren (keine neue Hardware-Beschaffung nötig, Konfig-Ergänzung genügt).
- **Kleine VM** im VÖB-Netz für den GitHub-Actions-Runner (2 vCPU, 4 GB RAM, Internet-Egress zu GitHub).
- **IP-Adresse** für den VÖB-seitigen VIP-Peer ist festgelegt und bei CCJ bekannt.
